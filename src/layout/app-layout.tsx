...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DojoInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/dojos` : null),
    () => getDojos({ tenant_id: session?.user?.tenantId }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Dojo',
      path: isTenantUser ? `/dojos/view/${data?.[0]?.id}` : '/dojos',
      entity: 'dojo',
    ...
    { name: 'Technique', path: '/techniques', entity: 'technique', service: AccessServiceEnum.PROJECT, icon: FiVideo },
